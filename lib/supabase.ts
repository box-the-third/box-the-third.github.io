...
// Same project + anon key as the legacy static pages (public/assets/js/db.js).
// IMPORTANT: use the DEFAULT auth storage options so the session is written
// to `sb-<ref>-auth-token` in localStorage, the exact key the CDN client used
// by the remaining static pages reads. A sign-in here is recognised there too.
